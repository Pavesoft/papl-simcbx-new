...
export function add(number1: string, number2 = '0') {
  ...
  const res = addCore(number1, number2)
...
export function pad(number1: string, number2: string) {
  ...
  if (length1 || length2) {
    if (length1 > length2) {
      parts2[1] =
        (parts2[1] ? parts2[1] : '') + new Array(Math.abs(length1 - length2) + 1).join('0')
    } else {
      parts1[1] =
        (parts1[1] ? parts1[1] : '') + new Array(Math.abs(length1 - length2) + 1).join('0')
    }
  }
...
function addCore(number1: string, number2: string) {
  ...
  for (let i = number1.length - 1; i >= 0; i--) {
    ...
    const temp = parseInt(number1[i]) + parseInt(number2[i]) + carry
    sum = (temp % 10) + sum
