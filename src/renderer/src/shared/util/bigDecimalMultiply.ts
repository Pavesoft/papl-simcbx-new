...
function adjustDecimal(number, decimal) {
  if (decimal == 0) return number
    ...
    number =
      decimal >= number.length ? new Array(decimal - number.length + 1).join('0') + number : number
    return (
      number.substr(0, number.length - decimal) +
      '.' +
      number.substr(number.length - decimal, decimal)
    )
  }
}
