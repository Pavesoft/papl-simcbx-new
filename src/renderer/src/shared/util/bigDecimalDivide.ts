...
export function bigDecimalDivide(
  dividend,
  divisor,
  precission = 8,
  ...
  const pt_dvsr = divisor.indexOf('.') > 0 ? divisor.length - divisor.indexOf('.') - 1 : -1

  divisor = trim(divisor.replace('.', ''))
  if (pt_dvsr >= 0) {
    const pt_dvnd = dividend.indexOf('.') > 0 ? dividend.length - dividend.indexOf('.') - 1 : -1
    ...
    if (pt_dvnd == -1) {
      ...
      if (pt_dvsr > pt_dvnd) {
        ...
        dividend = dividend.replace('.', '')
        const loc = dividend.length - pt_dvnd + pt_dvsr
