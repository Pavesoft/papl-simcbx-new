...
export function roundOff(
  ...
  if (!parts[1]) {
    ...
    return (
      (neg ? '-' : '') + partInt + '.' + parts[1] + new Array(n - parts[1].length + 1).join('0')
    )
  }

  partDec = parts[1].substring(0, n)
  const rem = parts[1].substring(n)

  if (rem && greaterThanFive(rem, partDec, neg, mode)) {
    partDec = increment(partDec)
    if (partDec.length > n) {
      return (
        (neg ? '-' : '') + increment(partInt, parseInt(partDec[0])) + '.' + partDec.substring(1)
      )
    }
  }
